...
def phantom(n=128, ellipses=None, nE=5):
    ...
    for ellip in ellipses:
        ...
        p[locs] = 255 * (0.5 + np.random.random() * 0.5)
    return p
...
def generate_one(noise_func=noisy):
    f_true = phantom()
...
def generate(n, folder_path, noise_type):
    start = time.time()
    ...
    for i in range(n):
        f_true, s, f_fbp, f_bp = generate_one(noise_func=noise_func)
...
def main():
    ...
    try:
        ...
        noise_type = str(
            input(
                "What noise type would you like? ([gaussian]/inpainting/undersampling) "
            )
        )
        if noise_type == "":
            noise_type = "gaussian"
        generate(num_samples, folder_path, noise_type=noise_type)
    except:
